...
def addProduct(request):
    if request.method == 'POST':
        ...
        price = request.POST['price']
        braand = Brand.objects.get(brand_name = brand)
        item = Product(name = productName,price = price,mobile=phone,state=state,city=city,owner = request.user,description = productDescription,condition = productCondition, brand = braand,image = "main_image/"+files)
        item.save()
        return redirect('product:product_list')
    else:
        if request.user.is_authenticated:
